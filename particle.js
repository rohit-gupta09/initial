...
const cross = loader.load('./images/star.png')
...
renderer.setSize(sizes.width, sizes.height)
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
renderer.setClearColor(new THREE.Color('#002200'), 0.7)
    ...
    if (mouseX > -2 && mouseX < 2) {
        if (mouseY > -2 && mouseY < 2) {
            ...
            if (mouseDown) {
                ...
                ring.position.x = pos.x * 0.2;
                ring.position.y = pos.y * 0.2;
                gsap.to(ring.scale, { x: 1.8, y: 1.8, z: 1.8 })
            } else {
                gsap.to(ring.position, { x: 0, y: 0, z: 0 })
                gsap.to(ring.scale, { x: 0.4, y: 0.4, z: 0.4 })
